...
do_read(int fd, void *buf, size_t nbytes)
{
	...
	if(cur_fd->f_mode!=1 && cur_fd->f_mode!=3 && cur_fd->f_mode!=5 && cur_fd->f_mode!=7) 
        {
           dbg(DBG_PRINT, "FD is not proper \n");
     ...
     */
        ...
        {
         dbg(DBG_PRINT, "It is a directory \n");
          fput(cur_fd);  
          return -EISDIR;
        }
	 ...
	 */
	 ...
	 fput(cur_fd);
	 dbg(DBG_PRINT, "Successful Read of the file\n");
...
{
	if(fd>=NFILES||fd<0||curproc->p_files[fd]==NULL){
		dbg(DBG_PRINT, "(GRADING2D 3.j): fd is not a valid file descriptor or is not open for reading\n");
		return -EBADF;
	}
...
do_mknod(const char *path, int mode, unsigned devid)
{
	...
	KASSERT(name!=NULL);
	dbg(DBG_PRINT, "(GRADING2D 3.b): The name component is present\n");
...
do_mkdir(const char *path)
{
	...
	{
		KASSERT(dir_namev_ret<0);
		dbg(DBG_PRINT,"(GRADING2D 3.c):Directory path has a problem\n");
		return dir_namev_ret;
	}
	...
	{
		KASSERT(!S_ISDIR(res_vnode->vn_mode));
		dbg(DBG_PRINT, "(GRADING2D 3.c): Some directory component is not a directory\n");
		vput(res_vnode);
		return -ENOTDIR;
	}
	...
	/*KASSERT(*name!=NULL);*/
	dbg(DBG_PRINT, "(GRADING2D 3.c): The name component is present\n");

	int lookup_ret = lookup(res_vnode,name,namelen,&temp_vnode);
	if(0==lookup_ret)
	{
		KASSERT(0==lookup_ret);
		dbg(DBG_PRINT, "(GRADING2D 3.c): The path exists. Would not be able to create the directory.\n");
		vput(res_vnode);
		vput(temp_vnode);
		return -EEXIST;
	}
...
do_rmdir(const char *path)
{
	...
	{
		KASSERT(dir_namev_ret<0);
		dbg(DBG_PRINT, "(GRADING2D 3.d): Some directory component does not exist\n");
		return dir_namev_ret;
	}
	else
	{
		...
		{
			KASSERT(!S_ISDIR(res_vnode->vn_mode));
			dbg(DBG_PRINT, "(GRADING2D 3.d): Some directory component is not a directory\n");
			vput(res_vnode);
			return -ENOTDIR;
		}
	}
	...
	if( (*(name+(strlen(name)-1))=='.') && (*(name+(strlen(name)-2))=='.')  ) 
		/*if(strcmp(*name,"..")==0)*/
	{
		dbg(DBG_PRINT,"Path had '..' as final component.\n");
		vput(res_vnode);
		return -ENOTEMPTY;
	}
	...
	if(lookup_ret<0)
	{
		...
		dbg(DBG_PRINT, "(GRADING2D 3.d): .\n");
		vput(res_vnode);
		return lookup_ret;
	}
	KASSERT(NULL!=res_vnode->vn_ops->rmdir);
	dbg(DBG_PRINT, "(GRADING2A 3.d): The function call exists.\n");
	int rmdir_ret=(res_vnode->vn_ops->rmdir)(res_vnode,name,namelen);
	vput(temp_vnode);
	vput(res_vnode);
	dbg(DBG_PRINT, "(GRADING2D 3.d): Directory successfully removed.\n");
	return rmdir_ret;
}
...
do_unlink(const char *path)
{
	...
	{
		KASSERT(lookup_ret<0);
		dbg(DBG_PRINT, "(GRADING2D 3.e): The path does not exist.\n");
		vput(res_vnode);
		return lookup_ret;
	}
	...
	if(S_ISDIR(temp_vnode->vn_mode))
	{
		...
		vput(temp_vnode);
		vput(res_vnode);
		dbg(DBG_PRINT, "(GRADING2D 3.e): Trying to remove directory.\n");
		return -EISDIR;
	}

	KASSERT(NULL!=res_vnode->vn_ops->unlink);
	dbg(DBG_PRINT, "(GRADING2A 3.e): The function call exists.\n");
	...
	vput(temp_vnode);
	vput(res_vnode);
	/*dbg(DBG_PRINT, "(GRADING2D 3.e): Directory successfully removed.\n");*/
	return unlink_ret;
}
...
do_getdent(int fd, struct dirent *dirp)
{
	...
	/*if(readdir_ret<0)
	{
        ...
        if(readdir_ret == 0)
	{
	        dbg(DBG_PRINT,"(GRADING2D 3.j) : Successfully ran getdent\n");
		fput(cur_f);
		return 0;
	}
	else
...
do_lseek(int fd, int offset, int whence)
{
	...
	if(whence != SEEK_SET && whence != SEEK_CUR && whence != SEEK_END){
	         fput(cur_file);
		dbg(DBG_PRINT, "(GRADING2D 3.k): Wrong whence type \n");
		return -EINVAL;
	}
	...
	dbg(DBG_PRINT, "(GRADING2D 3.k): Successful seek \n");
