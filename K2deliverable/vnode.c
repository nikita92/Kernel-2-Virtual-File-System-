 ...
 *  $Id: vnode.c,v 1.5 2014/09/21 19:44:42 cvsps Exp $
 */
...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        dbg(DBG_PRINT,"(GRADING2A 1.a): File is either a byte device or a block device.\n");
        if (S_ISCHR(file->vn_mode))
        {
          KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
          dbg(DBG_PRINT,"(GRADING2A 1.a): the device, operations and read are not null.\n");
        }  
        int bytes=0;
        if( S_ISCHR(file->vn_mode) )
        {
          dbg(DBG_PRINT,"(GRADING2B) : File is character device\n");
          bytes=file->vn_cdev->cd_ops->read( file->vn_cdev, offset , buf, count );
          return bytes;
        }
        /*dbg(DBG_PRINT,"(GRADING2B) : Success, special file read.\n");*/
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
        dbg(DBG_PRINT,"(GRADING2A 1.b): File is either a byte device or a block device.\n");
        if (S_ISCHR(file->vn_mode))
        {
          KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
          dbg(DBG_PRINT,"(GRADING2A 1.b): the device, operations and write are not null.\n");
        }  
        ...
        /*dbg(DBG_PRINT,"(GRADING2B) : Success, special file write.\n");*/
        /*else if ( S_ISBLK(file->vn_mode) )
        {
          return -ENOTSUP;
        }*/
