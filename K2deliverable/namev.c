...
lookup(vnode_t *dir, const char *name, size_t len, vnode_t **result)
{
	...
	if (len > NAME_LEN)
	{
		dbg(DBG_PRINT,"(GRADING2B): Name is too long.\n");
		return -ENAMETOOLONG;
	}
	...
	{
		dbg(DBG_PRINT,"(GRADING2B) : Current directory : %s \n",name);
		vref(dir);
		*result=dir;
		return 0;
	}
	...
	if(lookup_ret<0)
	{
		dbg(DBG_PRINT,"(GRADING2B) : The requested file does not exist : %s \n",name);
		return -ENOENT;
	}
          ...
          vnode_t *base, vnode_t **res_vnode)
{
	...
	if(pathname[0]=='/')
	{
		...
		dbg(DBG_PRINT,"(GRADING2B) : The requested directory starts with a root \n");
	}else if(base == NULL)
	{
	        dbg(DBG_PRINT,"(GRADING2B) : Current working directory will be used\n");
		vnode = curproc->p_cwd;
	}
	/*else
	{
	...
	{
		dbg(DBG_PRINT,"(GRADING2B) : There is atleast one level to go under\n");
	...
	*name = (char*)tempPathname;
	dbg(DBG_PRINT,"(GRADING2B) : Successfully finished dir_namev\n");
	return 0;
}
...
open_namev(const char *pathname, int flag, vnode_t **res_vnode, vnode_t *base)
{
	...
	if(dir_namev_ret<0)
	{
		...
		dbg(DBG_PRINT,"(GRADING2B):File path has a problem\n");
	...
	if(lookup_ret<0)
	{
		...
		if((flag & O_CREAT) && (lookup_ret==-ENOENT))
		{
			...
			if(create_ret<0)
			{
				dbg(DBG_PRINT, "(GRADING2C 1): The path exists. \n");
				vput(temp_vnode);
				return create_ret;
			}
		}
		...
		{
			dbg(DBG_PRINT,"(GRADING2B) : Lookup failed and mode is not create\n");
			vput(temp_vnode);
			return 	lookup_ret;					
		}
	}
	vput(temp_vnode);
	dbg(DBG_PRINT, "(GRADING2B): Successful openNamev. \n");
	return 0;
}
