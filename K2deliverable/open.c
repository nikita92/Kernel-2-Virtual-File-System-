...
do_open(const char *filename, int oflags)
{
	...
	if((oflags == (O_RDONLY|O_CREAT)) || (oflags == (O_RDONLY|O_TRUNC)) || oflags == (O_RDONLY)){
		...
		dbg(DBG_PRINT, "(GRADING2B):  File mode is FMODE_READ ");
	}else if( (oflags == (O_WRONLY|O_CREAT) || (oflags ==(O_WRONLY|O_TRUNC)) || (oflags ==O_WRONLY))){
		file->f_mode = FMODE_WRITE;
		dbg(DBG_PRINT, "(GRADING2B):  File mode is FMODE_WRITE ");
	}else if((oflags == (O_RDWR)) || (oflags == (O_RDWR|O_CREAT)) || (oflags == (O_RDWR|O_TRUNC))){
		file->f_mode = FMODE_WRITE|FMODE_READ;
		dbg(DBG_PRINT, "(GRADING2B):  File mode is FMODE_READ or FMODE_WRITE");
		...
		dbg(DBG_PRINT, "(GRADING2B):  File mode is FMODE_WRITE|FMODE_READ|FMODE_APPEND ");
	}else{
		dbg(DBG_PRINT,"(GRADING2B):  Invalid device");
		fput(file);
		return -EINVAL;
	}
	...
	if(open_ret!=0)
	{
			dbg(DBG_PRINT,"(GRADING2B): Lookup failed for the file");
			fput(file);
			return open_ret;
	}

	if(S_ISDIR(res_vnode->vn_mode) && ((oflags & 0x03)==O_WRONLY || (oflags & 0x03)==O_RDWR)){
		dbg(DBG_PRINT,"(GRADING2B):  The res_vnode refers to a character or block device");
		fput(file);
		vput(res_vnode);
		return -EISDIR;
	}
	...
	dbg(DBG_PRINT,"(GRADING2B):  Successfully opened the file");
	return next_fd;
